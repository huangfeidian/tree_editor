...
namespace spiritsaway::tree_editor
{
	...
	public:
		...
		basic_node(std::string _in_type, basic_node* _in_parent, node_idx_t _in_idx);
		...
		virtual basic_node* create_node(std::string _type, basic_node* _in_parent, node_idx_t _idx) = 0;
		virtual ~basic_node();
	};
